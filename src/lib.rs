...
// TO-DO: translate from Python
/*
// single-witness tracking.
// NOT TESTED
pub fn isum_1w(s: &[u8]) -> usize {
	let mut acc = 0;
	let mut cur_part_start = 0;
	let mut cur_windex: Option<usize> = None;
	let mut cur_witness: Option<u8> = None;
	let mut next_windex: Option<usize> = None;
	let mut next_witness: Option<u8> = None;
	let l  = s.len();
	while cur_part_start < l:
		...
		if target == s[hi - 1]:
			...
			if next_windex is not None:
				...
				// TO-DO: merge loops, because optimizer already dupes
				while lo < hi:
					mid = (lo + hi) >> 1
					if target != s[mid]:
						hi = mid
